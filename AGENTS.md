...
- Automatic enrichment uses official GMGN `POST https://openapi.gmgn.ai/v1/trenches` through `MarketDataProvider::fetch_markets`, with `X-APIKEY`, Unix-second `timestamp`, and a fresh UUID `client_id`. Only `GMGN_API_KEY` is configurable. Preserve the existing `GET /v1/token/info` implementation for explicit point queries; never use it as an automatic fallback for batch misses.
- One worker buffers up to 80 persisted candidates for at most five seconds, calls Trenches once, and joins exact mints. This buffer is only for API economy. No requests for empty batches, ranking, selection or per-token fallback. Matched snapshots append to `data/market-snapshots.jsonl`; unmatched candidates stay in discovery history. Recent-feed coverage is partial, not a guaranteed bulk mint lookup.
